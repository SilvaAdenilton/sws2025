...
# ============================================================
#  FUNÇÃO: Adicionar fundo com imagem externa
# ============================================================
def add_bg_from_local(image_file: str, opacity: float = 0.28):
    """Insere imagem de fundo com overlay escuro + blur suave."""
    try:
...
# ============================================================
#  CAMINHO FINAL DA IMAGEM — FUNCIONA 100%
# ============================================================
add_bg_from_local(
    r"C:\SWS_Assets\imagesbackground.png",
    opacity=0.28
)
...
valid_sheets = [
    ...
    if s.lower().strip() not in ["alarmes - cdg", "alarmes-cdg", "cdg"]
]

if not valid_sheets:
    st.error("❌ Nenhuma planilha válida encontrada (SWS1 ou SWS2).")
    st.stop()


# ============================================================
# SELETOR DESTACADO DE PLANILHA
# ============================================================
st.markdown("""
<div style="
...
if eff_col: df[eff_col] = pd.to_numeric(df[eff_col], errors="coerce").fillna(0)
if not_eff_col: df[not_eff_col] = pd.to_numeric(df[not_eff_col], errors="coerce").fillna(0)
...
def clear(val):
    return [] if "Todos" in val else val


# Serial
if serial_col:
    ops = ["Todos"] + sorted(df[serial_col].dropna().astype(str).unique())
    serial_sel = clear(st.sidebar.multiselect("Serial", ops, default=["Todos"]))
else:
    serial_sel = []
...
# Erros
if error_col:
    ops = ["Todos"] + sorted([str(e).strip() for e in df[error_col].unique() if str(e).strip() != ""])
